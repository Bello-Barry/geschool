...
export async function POST(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const storagePath = `assignments/${assignmentId}/${Date.now()}-${Math.random().toString(36).slice(2)}.${fileExt}`;
  ...
  if (dbError) {
    await adminClient.storage.from("assignment-attachments").remove([storagePath]);

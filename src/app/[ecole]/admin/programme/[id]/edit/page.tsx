...
export default async function EditProgrammePage({ params }: { params: Promise<{ ecole: string; id: string }> }) {
  ...
  const [{ data: entry }, { data: subjects }, { data: classes }, { data: terms }] = await Promise.all([
    supabase.from("programmes").select("*").eq("id", id).eq("school_id", schoolId).single(),
    ...
    <div className="space-y-6">
      <div className="flex items-center gap-4">
        <Button asChild variant="ghost" size="icon">
          <Link href={`/${ecole}/admin/programme`}>
